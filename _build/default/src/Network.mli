(**Representation of static social network. 
  ...
  The first value of the list is the x value, the second is the y value.
  Precondition: position has exactly two elements *)
...
(** Raised when a non-existing edge is encountered*)
  ...
  (** [from_json j] is the network that [j] represents. Requires: [j] is
    a valid JSON network representation. *)
  ...
  Raised [UnknownPerson p1] if p1 does not exist in [net].
  Raises [UnknownEdge (p1,p2)] if an edge between [p1] and [p2] does 
  not exist.*)